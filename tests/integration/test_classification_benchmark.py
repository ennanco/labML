...
def test_classification_benchmark_dry_run_does_not_write_output(tmp_path: Path) -> None:
    data = pd.DataFrame(
        {
            "f1": [1.0, 1.1, 2.0, 2.2, 3.0, 3.2, 4.0, 4.2],
            "f2": [0.2, 0.3, 0.8, 0.9, 1.5, 1.6, 2.1, 2.2],
    ...
    config_path = tmp_path / "benchmark_cls_dry_run.toml"
    config_path.write_text(
        "[input]\n"
        'source = "external"\n'
        f'data_path = "{data_path.name}"\n'
        ...
        "max_iter = [500]\n\n"
        "[output]\n"
        'file = "cls_dry_run_results.xlsx"\n',
        encoding="utf-8",
    )

    run_benchmark(config_path, task="classification", dry_run=True)
    assert not (tmp_path / "cls_dry_run_results.xlsx").exists()
